...
public:
	...
	Graph(const char* name, unsigned int seconds, unsigned int samplesPerSecond);
	~Graph();
...
private:
	...
	inline float GetSample() {
		Graph::tempString = m_Name;
		m_LatestSample = m_GetSample();
...
private:
	...
	std::function<float()> m_GetSample;//The function that gets the next sample
